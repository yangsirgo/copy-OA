...
define(function(require,exports) {
    ...
    //角色级联事件
    $("#table_conCurrentRole[type='simple'] tr select:even").live("change",function(){
        //绑定角色
        act_common.wf_bindRole($(this).val(), $(this).next());
    })
    //删除角色
    $("#table_conCurrentRole[type='simple'] img").live("click",function(){
        var trObj = $(this).parents("#table_conCurrentRole tr");
        var trlength = $("#table_conCurrentRole tr").length; //获取一共有多少个角色
        var delNum = $("#table_conCurrentRole tr").index(trObj) + 1;//获取要删除第几个元素值
        var count = trlength - delNum; //获取删除后还有多少个角色
        if (count > 0) {//当前删除的不是最后一个角色，进行重新排列
            $("#table_conCurrentRole tr:gt("+(delNum-1)+")").each(function(i){
                if((delNum + i) >9)
    ...
    //添加并签角色
    $("#a_AddConCurrentRoleSimple").live("click",function(){
        addRole();
    })
    function addRole(){
        //判断table对象下有多少个tr元素，一个tr元素代表一个角色
        var count = $("#table_conCurrentRole tr").length + 1;
        //添加角色元素及值
        var tr = $("<tr></tr>");
        ...
        if(count > 9)
            ...
            td.append("<span>角色&nbsp;&nbsp;" + count + "&nbsp;：</span>");
        td.append("&nbsp;<select><option value='2'>专项角色</option></select>&nbsp;");
        var select = $("<select></select>");
        td.append(select);
        td.append("&nbsp;<img src=\""+i8_session.resWfHost+"default/images/design/graph/delete.png\"/>");
        tr.append(td);
        $("#table_conCurrentRole").append(tr);
        //绑定角色
        var thisTr = $("#table_conCurrentRole tr").eq(count-1);
        act_common.wf_bindRole($(thisTr).find("select:first").val(), $(thisTr).find("select:last"));
    }
    //提交方法
    $("#txt_conCurrentSubmitSimple").live("click",function(){
        var txt_conCurrentTitle = $("#txt_conCurrentTitle"); //判断节点名称是否为空
        var table_conCurrentRole = $("#table_conCurrentRole"); //获取角色table,
        if ($.trim(txt_conCurrentTitle.val()) == "") {
            i8ui.alert({title:"环节说明不能为空!"});
            return false;
        }
        var charater_list = /^[\u4e00-\u9fa5_a-zA-Z0-9\(\)]+$/ig;
        if (!charater_list.test($.trim(txt_conCurrentTitle.val()))) {
            i8ui.alert({title:"环节说明只允许中英文字符和数字!"});
            return;
        }
        //判断table下是否存在tr元素，没有则代表没有选择角色,一个tr元素代表一个角色
        if ($("#table_conCurrentRole tr").length == 0) {
        ...
        var Approver = "";
        var isExist = true;
        $("#table_conCurrentRole tr").each(function(){
        ...
        //封装数据
        act_common.wf_addActity(wf_graph.getSelectionCell().id, $.trim(txt_conCurrentTitle.val()), 2, 0, Approver, ApproverType, "");
        console.log(act_common.wf_ProcActityArray);

        var cell = wf_graph.getSelectionCell(); //获取当前的cell
        cell.setAttribute("value", $.trim(txt_conCurrentTitle.val())); //赋值
    ...
    function giveAct(listOrderNum){
        if (listOrderNum != -1) {//不等于代表存储过该对象，进行赋值
            ...
            $("#table_conCurrentRole tr").remove();//删除
            ...
            if (ApproverRoleType.indexOf("|") > 0) {
                ...
                for (var i = 0; i < ApproverRoleTypeList.length; i++) {//根据选择的角色循环赋值
                    addRole(); //动态创建角色
                    //赋值
                    var trObj = $("#table_conCurrentRole tr").eq(i);
                ...
                addRole(); //动态创建角色
                //赋值
                var trObj = $("#table_conCurrentRole tr:first")
                var roleType = $(trObj).find("select:first");
                var roleNodes = $(trObj).find("select:last");
