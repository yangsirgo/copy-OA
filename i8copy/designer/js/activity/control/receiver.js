...
define(function(require,exports) {
    ...
    //角色级联事件
    $("#table_ReceiverRole tr select:even").live("change",function(){
        //绑定角色
        act_common.wf_bindRole($(this).val(), $(this).next());
    })

    //删除角色
    $("#table_ReceiverRole img").live("click",function(){
        var trObj = $(this).parents("#table_ReceiverRole tr");
        var trlength = $("#table_ReceiverRole tr").length; //获取一共有多少个角色
        var delNum = $("#table_ReceiverRole tr").index(trObj) + 1;//获取要删除第几个元素值
        var count = trlength - delNum; //获取删除后还有多少个角色
        if (count > 0) {//当前删除的不是最后一个角色，进行重新排列
            $("#table_ReceiverRole tr:gt("+(delNum-1)+")").each(function(i) {
                if ((delNum + i) > 9)
    ...
    //添加并签角色
    $("#a_AddReceiverRole").live("click",function(){
        addRole();
    })
    function addRole(){
        //判断table对象下有多少个tr元素，一个tr元素代表一个角色
        var count = $("#table_ReceiverRole tr").length + 1;
        //添加角色元素及值
        var tr = $("<tr></tr>");
        ...
        tr.append(td);
        $("#table_ReceiverRole").append(tr);
        //绑定角色
        var thisTr = $("#table_ReceiverRole tr").eq(count-1);
        act_common.wf_bindRole($(thisTr).find("select:first").val(), $(thisTr).find("select:last"));
    }

    //提交方法
    $("#btn_ReceiverSubmit").live("click",function(){
        var txt_ReceiverTitle = $("#txt_ReceiverTitle"); //判断节点名称是否为空
        var table_ReceiverRole = $("#table_ReceiverRole"); //获取角色table,
        var _txt_ReceiverTitleValue = $.trim(txt_ReceiverTitle.val());
        if (_txt_ReceiverTitleValue == "") {
            i8ui.alert({title:"环节说明不能为空!"});
            return false;
        }
        //判断table下是否存在tr元素，没有则代表没有选择角色,一个tr元素代表一个角色
        if ($("#table_ReceiverRole tr").length == 0) {
        ...
        var Approver = "";
        var isExist = true;
        $("#table_ReceiverRole tr").each(function(){
        ...
        //封装数据
        act_common.wf_addActity(wf_graph.getSelectionCell().id, _txt_ReceiverTitleValue, 3, 0, Approver, ApproverType, "");
        var cell = wf_graph.getSelectionCell(); //获取当前的cell
        cell.setAttribute("value", _txt_ReceiverTitleValue); //赋值
    ...
    function giveAct(listOrderNum){
        if (listOrderNum != -1) {//不等于代表存储过该对象，进行赋值
            ...
            $("#table_ReceiverRole tr").remove();//删除
            ...
            if (ApproverRoleType.indexOf("|") > 0) {
                ...
                for (var i = 0; i < ApproverRoleTypeList.length; i++) {//根据选择的角色循环赋值
                    addRole(); //动态创建角色
                    //赋值
                    var trObj = $("#table_ReceiverRole tr").eq(i);
                ...
                addRole(); //动态创建角色
                //赋值
                var trObj = $("#table_ReceiverRole tr:first")
                var roleType = $(trObj).find("select:first");
                var roleNodes = $(trObj).find("select:last");
