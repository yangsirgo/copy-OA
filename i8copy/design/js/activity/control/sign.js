...
define(function(require,exports) {
    ...
    //角色级联事件
    $("#table_SignRole tr select:even").live("change",function(){
        //绑定角色
        act_common.wf_bindRole($(this).val(), $(this).next());
    })

    //删除角色
    $("#table_SignRole img").live("click",function(){
        var trObj = $(this).parents("#table_SignRole tr");
        var trlength = $("#table_SignRole tr").length; //获取一共有多少个角色
        var delNum = $("#table_SignRole tr").index(trObj) + 1;//获取要删除第几个元素值
        var count = trlength - delNum; //获取删除后还有多少个角色
        if (count > 0) {//当前删除的不是最后一个角色，进行重新排列
            $("#table_SignRole tr:gt("+(delNum-1)+")").each(function(i){
    ...
    //添加并签角色
    $("#a_AddSignRole").live("click",function(){
        addRole();
    })
    function addRole(){
        //判断table对象下有多少个tr元素，一个tr元素代表一个角色
        var count = $("#table_SignRole tr").length + 1;
        //添加角色元素及值
        var tr = $("<tr></tr>");
        ...
        tr.append(td);
        $("#table_SignRole").append(tr);
        //绑定角色
        var thisTr = $("#table_SignRole tr").eq(count-1);
        act_common.wf_bindRole($(thisTr).find("select:first").val(), $(thisTr).find("select:last"));
    }

    //提交方法
    $("#txt_SignSubmit").live("click",function(){
        var txt_SignTitle = $("#txt_SignTitle"); //判断节点名称是否为空
        var table_SignRole = $("#table_SignRole"); //获取角色table,
        ...
        var Approver = "";
        var isExist = true;
        $("#table_SignRole tr").each(function(){
        ...
        //封装数据
        act_common.wf_addActity(wf_graph.getSelectionCell().id, _txt_SignTitleValue, 3, 0, Approver, ApproverType, "");
        ...
        if (listOrderNum != -1) {//不等于代表存储过该对象，进行赋值
            ...
            if (ApproverRoleType.indexOf("|") > 0) {
                ...
                for (var i = 0; i < ApproverRoleTypeList.length; i++) {//根据选择的角色循环赋值
                    addRole(); //动态创建角色
                    //赋值
                    var trObj = $("#table_SignRole tr").eq(i);
                ...
                addRole(); //动态创建角色
                //赋值
                var trObj = $("#table_SignRole tr:first")
                var roleType = $(trObj).find("select:first");
                var roleNodes = $(trObj).find("select:last");
